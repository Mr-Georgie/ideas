...
if (!url || !key) {
  console.error('[IDIAS] Missing REACT_APP_SUPABASE_URL or REACT_APP_SUPABASE_PUBLISHABLE_KEY');
}
...
export const VARIANTS = ['manifesto', 'sticky', 'polaroid', 'receipt', 'halftone', 'newsclip'];
...
export function transformIdea(row, index = 0) {
  ...
  const totalIdeas = (author.shipped_count || 0) + (author.cooking_count || 0) + (author.dropped_count || 0);

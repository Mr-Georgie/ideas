...
export function useIdeas(filter = 'ALL') {
  ...
  const load = useCallback(async () => {
    ...
    if (err) { console.error('[IDIAS] ideas: load error', err.message); setError(err.message); setLoading(false); return; }
    console.log('[IDIAS] ideas: loaded', data?.length ?? 0, 'ideas');
    setIdeas((data || []).map((row, i) => transformIdea(row, i)));
    setLoading(false);
  }, [filter]);
  ...
  useEffect(() => {
    const ideasSub = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'ideas' }, () => {
        console.log('[IDIAS] ideas: realtime INSERT → refetching feed');
        load();
      })
      .subscribe((status) => console.log('[IDIAS] ideas channel status →', status));
    ...
    const reactionsSub = supabase
      ...
      .subscribe((status) => console.log('[IDIAS] reactions-feed channel status →', status));

...
function Step0({ data, setField }) {
  return (
    <div className="fade-in">
      ...
      <div className="f-display" style={{ fontSize: 36, lineHeight: 0.94, marginBottom: 18 }}>
        how <span style={{ color: 'var(--pink)' }}>baked</span><br />is it?
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {HEAT_OPTS.map((o, i) => {
          const active = data.heat === o.k;
          return (
            <button key={o.k} onClick={() => setField('heat', o.k)} className="tap" style={{
              background: active ? o.color : 'var(--paper)',
              border: '2.5px solid var(--ink)', padding: 14, textAlign: 'left',
              boxShadow: active ? '4px 4px 0 var(--ink)' : '2px 2px 0 var(--ink-3)',
              transform: `rotate(${i % 2 === 0 ? -0.8 : 0.8}deg)`,
            }}>
              <div className="f-display" style={{ fontSize: 22, lineHeight: 1, color: 'var(--ink)' }}>{o.label}</div>
              <div className="f-body" style={{ fontSize: 12, marginTop: 6, color: active ? 'var(--ink)' : 'var(--ink-2)' }}>{o.desc}</div>
            </button>
          );
        })}
      </div>
...
function Step1({ data, setField }) {
  ...
  return (
    <div className="fade-in">
      ...
      <div className="f-display" style={{ fontSize: 36, lineHeight: 0.94, marginBottom: 14 }}>
        what's<br />the <span style={{ color: 'var(--blue)' }}>idea?</span>
      </div>
      <textarea
        value={data.title}
        ...
        style={{
          ...
          fontFamily: "'Caprasimo', serif", fontSize: 22, lineHeight: 1.1,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
        ...
        <span className="f-mono" style={{ fontSize: 10, color: len > 5 ? 'var(--lime)' : 'var(--ink-3)' }}>
          {len > 5 ? '✓ READY' : 'KEEP GOING'}
        </span>
      </div>
      <div style={{ marginTop: 18 }}>
        <div className="f-mono" style={{ fontSize: 10, color: 'var(--ink-2)', letterSpacing: '0.1em', marginBottom: 8 }}>STUCK? STEAL A PROMPT ↓</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {PROMPTS.slice(0, 3).map((p, i) => (
            <button key={p} onClick={() => setField('title', p)} className="tap" style={{
              textAlign: 'left', background: 'var(--paper-2)',
              border: '1.5px dashed var(--ink-2)', padding: '8px 10px',
              fontFamily: "'JetBrains Mono', monospace", fontSize: 12,
              color: 'var(--ink-2)', transform: `rotate(${i * 0.4 - 0.4}deg)`,
            }}>→ {p}</button>
          ))}
        </div>
      </div>
...
function Step2({ data, setField }) {
  ...
  return (
    <div className="fade-in">
      ...
      <div className="f-body" style={{ fontSize: 13, color: 'var(--ink-2)', marginBottom: 10 }}>
        2-4 sentences. Don't pitch it. Just describe it like you would to a friend on a walk.
      </div>
      <textarea
        value={data.body}
      ...
      />
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
        <span className="f-mono" style={{ fontSize: 10, color: 'var(--ink-3)' }}>{data.body.length} chars</span>
        <span className="f-mono" style={{ fontSize: 10, color: 'var(--ink-3)' }}>~{readSec}s read</span>
      </div>
    </div>
  );
}
...
function Step3({ data, toggleTag }) {
  return (
    <div className="fade-in">
      ...
      <div className="f-display" style={{ fontSize: 36, lineHeight: 0.94, marginBottom: 6 }}>
        tag it<br /><span style={{ color: 'var(--mustard)' }}>(max 4)</span>
      </div>
      ...
      <div style={{ marginTop: 22 }}>
        <div className="f-mono" style={{ fontSize: 10, color: 'var(--ink-2)', letterSpacing: '0.1em', marginBottom: 6 }}>
          SELECTED ({data.tags.length}/4)
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, minHeight: 28 }}>
          {data.tags.length === 0 && <span className="f-mono" style={{ fontSize: 11, color: 'var(--ink-3)' }}>none yet</span>}
          {data.tags.map(t => <Tag key={t} color="var(--ink)" filled tilt={-1}>#{t}</Tag>)}
        </div>
      </div>
    </div>
  );
    ...
    <div className="fade-in">
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{
          ...
          padding: 16, boxShadow: `5px 5px 0 ${heatColor}, 5px 5px 0 3px var(--ink)`,
          transform: 'rotate(-1deg)',
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Avatar name={displayName.slice(0, 2).toUpperCase()} size={28} color={profile?.avatar_color || heatColor} shape="soft" />
              <div className="f-body" style={{ fontWeight: 700, fontSize: 12 }}>{displayName}</div>
            </div>
          ...
          <div className="f-body" style={{ fontSize: 13, color: 'var(--ink-2)', marginTop: 8, lineHeight: 1.45 }}>
            {data.body || 'no body yet…'}
          </div>
          <div style={{ display: 'flex', gap: 5, marginTop: 12, flexWrap: 'wrap' }}>
            {data.tags.map(t => <Tag key={t} color="var(--ink)">#{t}</Tag>)}
          </div>
        </div>
      </div>
      <div style={{ marginTop: 22 }}>
        <PostIt color="var(--mustard)" tilt={2} style={{ fontSize: 12 }}>
          <div className="f-mono" style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', marginBottom: 4 }}>FYI</div>
          when you pin this, it joins the bulletin. responses are constructive only.
        </PostIt>
      </div>
    </div>
  );
...
export function PostFlow({ onClose, onPosted }) {
  ...
  const toggleTag = (t) => setData(d => ({
    ...d,
    tags: d.tags.includes(t) ? d.tags.filter(x => x !== t) : d.tags.length < 4 ? [...d.tags, t] : d.tags,
  }));
  ...
  const submit = async () => {
    if (!session?.user) { setErr('you need to sign in first'); return; }
    setSubmitting(true);
    setErr('');

    const totalIdeas = (profile?.shipped_count || 0) + (profile?.cooking_count || 0) + (profile?.dropped_count || 0);
    ...
    const counterField = data.heat === 'shipped' ? 'shipped_count'
      : data.heat === 'dropped' ? 'dropped_count'
      : 'cooking_count';
  ...
  return (
    <div className="paper-bg" style={{ minHeight: '100%', position: 'relative', display: 'flex', flexDirection: 'column' }}>
      {/* header */}
      <div style={{ padding: '14px 16px 12px', borderBottom: '2px solid var(--ink)', background: 'var(--paper)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <button onClick={onClose} className="tap" style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'var(--ink)' }}>
            <Icon name="close" size={18} />
            <span className="f-mono" style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em' }}>NEVERMIND</span>
          </button>
          <div className="f-mono" style={{ fontSize: 10, fontWeight: 700, color: 'var(--ink-3)', letterSpacing: '0.12em' }}>
            STEP {step + 1} / {total}
          </div>
        </div>
      ...
      {/* action bar */}
      <div style={{ padding: '12px 16px 16px', borderTop: '2px solid var(--ink)', background: 'var(--paper)', display: 'flex', gap: 10 }}>
        {step > 0 && (
          <button onClick={prev} className="tap" style={{
            padding: '11px 16px', background: 'transparent', border: '2px solid var(--ink)',
            fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, fontSize: 12, letterSpacing: '0.1em',
          }}>← BACK</button>
        )}
        {step < total - 1 && (
          <button onClick={next} disabled={!canAdvance()} className="tap" style={{
        ...
        {step === total - 1 && (
          <button onClick={submit} disabled={submitting} className="tap" style={{
            flex: 1, padding: '11px 16px',
            background: 'var(--lime)', border: '2px solid var(--ink)',
            boxShadow: '4px 4px 0 var(--ink)',
            fontFamily: "'JetBrains Mono', monospace", fontWeight: 700, fontSize: 12, letterSpacing: '0.1em',
            opacity: submitting ? 0.6 : 1,
          }}>{submitting ? 'PINNING…' : '✺ PIN IT TO THE BOARD'}</button>
        )}
      </div>
    </div>
  );

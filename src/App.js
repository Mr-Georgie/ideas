        ...
        if (t.center) {
          return (
            <button
              key="_post"
              onClick={openPost}
              className="tap animate-bounce-slow hover:animate-bounce-fast"
              style={{
                background: "var(--pink)",
                color: "var(--paper)",
        ...
        return (
          <button
            ...
            style={{
              ...
              padding: "6px 10px",
              position: "relative",
              color: active ? "var(--pink)" : "var(--ink-2)",
              minWidth: 56,
            }}
          >
            <Icon
...
function PostedSplash() {
  return (
    ...
    >
      <div
        className="f-display animate-rubber-band"
        style={{
          fontSize: 64,
          lineHeight: 0.92,
          color: "var(--ink)",
...
const PALETTE_META = {
  classic: {
    label: "CLASSIC ZINE",
    dots: ["#f3ede0", "#ff3d7f", "#f5c518", "#3d5bff"],
  },
  riso: {
    label: "RISO PRINT",
    dots: ["#f7f1e8", "#ff5252", "#ffcb05", "#005bbb"],
  },
  acid: {
    label: "ACID NOTEBOOK",
    dots: ["#ecf3d8", "#ff2d95", "#fff200", "#00d4ff"],
  },
  bruise: {
    label: "BRUISE",
    dots: ["#1a1410", "#ff3d7f", "#f5c518", "#a78bfa"],
  },
};
      ...
      >
        ...
        >
          {PALETTE_META[palette].dots.map((c, i) => (
            <div
              key={i}
              style={{ width: 7, height: 7, background: c, borderRadius: 1 }}
            />
          ))}
        </div>
      </button>
...
function AppInner() {
  ...
  const [remixOf, setRemixOf] = useState(null);
  ...
  const needsAuth =
    !session || (session && profile !== null && !profile?.username);
        ...
        <DesktopShell
          ...
          onOpenOriginal={async (ideaId) => {
            ...
            const { data, error } = await supabase
              ...
              .single();
            if (!error && data) {
              setOpenIdea(transformIdea(data, 0));
            }
          }}
          newCount={newCount}
        />
  ...
  return (
    ...
    >
      ...
      {!needsAuth && posting && (
        ...
        >
          <PostFlow
            onClose={() => { setPosting(false); setRemixOf(null); }}
            onPosted={handlePosted}
            remixOf={remixOf}
          />
        </div>
